...
const DemandProjection = ({ setSelected, activeProject }) => {
    ...
    const demandProcess = processes['demand'];
    const isModalVisible = activeModal === 'demand';
    const isForecasting = demandProcess?.status === 'running';
    ...
    const handleStartForecast = async (config) => {
        try {
            ...
            registerEventSource('demand', eventSource);

            eventSource.onopen = () => {
                updateLogs('demand', {
                    type: 'info',
                    text: 'Connection to server established. Waiting for process to start...'
                });
            };
            ...
            eventSource.addEventListener('progress', (event) => {
                ...
                if (data.total_sectors) {
                    updateTaskProgress('demand', {
                        total: data.total_sectors,
                    });
                    updateLogs('demand', {
                        type: 'info',
                        text: `Forecast initialized for ${data.total_sectors} sectors.`
                    });
                }
            ...
            eventSource.addEventListener('sector_completed', (event) => {
                ...
                updateTaskProgress('demand', {
                    current: (demandProcess?.taskProgress?.current || 0) + 1,
                });
            });
            ...
            eventSource.addEventListener('end', (event) => {
                ...
                if (data.status === 'completed') {
                    updateStatus('demand', 'completed');
                    updateProgress('demand', {
                        percentage: 100,
                        message: 'Forecast process finished!'
                    });
                    ...
                    if (data.result) {
                        ...
                        updateLogs('demand', {
                            type: 'success',
                            text: `✅ Forecast completed: ${successfulSectors} sectors successful, ${failedSectors} failed.`
                        });
                    } else {
                        updateLogs('demand', {
                            type: 'success',
                    ...
                    toast.success('Demand forecast completed successfully!');
                } else {
                    updateStatus('demand', 'failed', data.error || 'Forecast failed');
                    updateLogs('demand', {
                        type: 'error',
                        text: `❌ Forecast failed: ${data.error || 'An unknown error occurred.'}`
                    });
                    toast.error('Demand forecast failed');
                }
            ...
            eventSource.onerror = () => {
                updateStatus('demand', 'failed', 'Connection error');
                updateLogs('demand', {
                    type: 'error',
                    text: 'Connection to the server was lost.'
                });
                toast.error('Connection error during forecast');
                eventSource.close();
            };
    ...
    return (
        <div className="h-full w-full bg-slate-50 text-slate-800 p-1 font-sans flex flex-col text-xs">
            ...
            <ConfigureForecast
                ...
                onApply={handleStartForecast}
            />

            {/* Process Modal with unified notification system */}
            {isModalVisible && demandProcess && (
                <ProcessModal
                    process={demandProcess}
                    onClose={() => {
                        handleCloseModal();
                        if (demandProcess.status === 'completed') {
                            handleForecastComplete();
                        }
                    }}
                    onMinimize={hideModal}
                    onStop={null} // No stop endpoint for demand forecasting
                />
            )}

            {/* Floating Indicator when minimized */}
            {!isModalVisible && demandProcess?.status === 'running' && (
                <FloatingProcessIndicator
                    process={demandProcess}
                    onClick={() => showModal('demand')}
                    onStop={null}
                />
            )}
        </div>
