...
def load_color_configuration(active_project, current_color_settings):
    ...
    from dash.services.local_service import LocalService
    ...
    try:
        ...
        sectors_result = service.get_sectors(project_path)
        sectors = sectors_result.get('sectors', [])

        if not sectors:
            sector_grid = html.Div([
                html.P('No sectors found in project.', style={
                    'textAlign': 'center',
                    'color': '#6B7280'
                })
            ], style={
        ...
        try:
            ...
            if color_result and 'colors' in color_result:
                ...
                model_colors = saved_colors.get('models', default_models)
            else:
                # Generate default colors for sectors
                sector_colors = {}
                for i, sector in enumerate(sectors):
                    sector_colors[sector] = default_sector_colors[i % len(default_sector_colors)]
                model_colors = default_models
        except:
            # Generate default colors for sectors
            sector_colors = {}
            for i, sector in enumerate(sectors):
