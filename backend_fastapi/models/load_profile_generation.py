...
#             # Load Duration Curve
#             percent_bins = np.arange(1, 101)  # 1,2,...,100
...
#                 else:
#                     q = 100.0 - percent_bins  # e.g. p=1 -> q=99 -> top 1% value
...
"""
Optimized Load Profile Generation System
Maintains original JSON input structure and Excel output format
Performance and memory optimizations applied
"""
...
class ProgressReporter:
    """Optimized progress reporting for WebSocket integration"""
...
class OptimizedPatternExtractor:
    """Optimized pattern extraction focusing on essential patterns only"""
    ...
    def _get_fiscal_year(self, month, year):
        """Cached fiscal year calculation"""
        return year + 1 if month >= 4 else year
    
    def _prepare_data(self):
        """Optimized data preparation using vectorized operations"""
        print("\nPreparing historical data...", file=sys.stderr)
    ...
    def extract_essential_patterns(self):
        """Extract only essential patterns needed for generation"""
        print("\nExtracting essential patterns...", file=sys.stderr)
        
        # Hourly patterns by day type (vectorized aggregation)
        self.patterns['hourly'] = self._extract_hourly_patterns()
        
        # Monthly and seasonal patterns
        self.patterns['monthly'] = self._extract_monthly_patterns()
    ...
    def _extract_monthly_patterns(self):
        """Extract monthly demand patterns"""
        monthly_stats = self.data.groupby('fiscal_month')['demand'].agg([
            'mean', 'std', 'min', 'max', 'median'
        ]).reset_index()
    ...
    def _extract_day_type_factors(self):
        """Extract day type reduction factors"""
    ...
    def _extract_stl_components(self):
        """Extract STL decomposition components if needed"""
        try:
            ...
            return {
                ...
                'seasonal': decomposition.seasonal.values[:168],  # One week pattern
...
class OptimizedLoadProfileGenerator:
    """Optimized load profile generator with improved performance"""
    ...
    def _precompute_values(self):
        ...
        # Precompute fiscal month mapping
        self.fiscal_month_names = {
            1: 'Apr', 2: 'May', 3: 'Jun', 4: 'Jul', 5: 'Aug', 6: 'Sep',
            7: 'Oct', 8: 'Nov', 9: 'Dec', 10: 'Jan', 11: 'Feb', 12: 'Mar'
        }
        
        # Precompute season mapping array
        self.season_map = {
            3: 'Summer', 4: 'Summer', 5: 'Summer', 6: 'Summer',
            7: 'Monsoon', 8: 'Monsoon', 9: 'Monsoon',
            10: 'Post-monsoon', 11: 'Post-monsoon',
    ...
    def generate_profile(self):
        """Generate load profile using optimized methods"""
        print("\n" + "="*60, file=sys.stderr)
        print(f"OPTIMIZED PROFILE GENERATION - {self.method.upper()}", file=sys.stderr)
        print("="*60, file=sys.stderr)
        
        if self.progress:
            self.progress.start_process(6, "Optimized Load Profile Generation")
        ...
        # Generate demand based on method
        if self.method == 'stl_decomposition' and STL_AVAILABLE and 'stl' in self.patterns:
            if self.progress:
                self.progress.update_progress("Generating STL-based profile")
            profile_df = self._generate_stl_profile(profile_df)
        else:
            if self.progress:
                self.progress.update_progress("Generating normalized pattern profile")
            profile_df = self._generate_normalized_profile(profile_df)
        
        # Apply constraints and scaling
        if self.progress:
            self.progress.update_progress("Applying constraints and scaling")
        profile_df = self._apply_constraints_and_scaling(profile_df)
    ...
    def _load_from_forecast(self):
        ...
        try:
            if os.path.exists(forecast_path):
                ...
                # Find columns
                year_col = next((col for col in forecast_data.columns 
                                if 'year' in str(col).lower() or 'fy' in str(col).lower()), None)
                demand_col = next((col for col in forecast_data.columns 
                                  if 'total' in str(col).lower() and 
                                  ('demand' in str(col).lower() or 'energy' in str(col).lower())), None)
                
                if year_col and demand_col:
                    for _, row in forecast_data.iterrows():
                        try:
                            year = int(float(str(row[year_col]).replace('FY', '').strip()))
                            demand = float(row[demand_col])
                            if self.start_year <= year <= self.end_year:
                                self.demand_targets[year] = demand
                        except (ValueError, TypeError):
    ...
    def _generate_default_targets(self):
        ...
        # Get base year total from historical data
        historical_data = self.template_data.get('Past_Hourly_Demand', pd.DataFrame())
        if not historical_data.empty:
            base_total = historical_data['demand'].sum() / len(historical_data['demand'].unique())
        else:
            base_total = 1000000  # Default 1 million MWh
    ...
    def _create_profile_structure(self):
        """Create optimized profile DataFrame structure"""
        # Generate date range
        ...
        # Vectorized fiscal month calculation
        profile_df['fiscal_month'] = ((profile_df['Month'] - 4) % 12) + 1
        
        # Vectorized day type classification
        profile_df['is_weekend'] = profile_df['DayOfWeek'].isin([5, 6]).astype(int)
    ...
    def _generate_normalized_profile(self, profile_df):
        """Generate profile using optimized normalized pattern method"""
        # Extract base year curve
        base_curve = self._extract_base_year_curve()
        
        # Normalize base curve
        base_min = base_curve['demand'].min()
        base_max = base_curve['demand'].max()
        base_range = base_max - base_min
        
        if base_range > 0:
            base_curve['normalized'] = (base_curve['demand'] - base_min) / base_range
        else:
            base_curve['normalized'] = 0.5
        
        # Add temporal features to base curve for mapping
        base_curve['hour'] = base_curve['datetime'].dt.hour
        base_curve['fiscal_month'] = ((base_curve['datetime'].dt.month - 4) % 12) + 1
        base_curve['day'] = base_curve['datetime'].dt.day
        
        # Initialize demand array
        demand = np.zeros(len(profile_df))
        
        # Vectorized processing for each year
        for year in range(self.start_year, self.end_year + 1):
            year_mask = profile_df['Fiscal_Year'] == year
            if not year_mask.any():
                continue
            
            year_data = profile_df[year_mask]
            
            # Calculate growth factor
            growth_factor = self._calculate_growth_factor(year)
            
            # Map to base year patterns (vectorized)
            for month in range(1, 13):
                month_mask = year_mask & (profile_df['fiscal_month'] == month)
                ...
                # Get base month data
                base_month = base_curve[base_curve['fiscal_month'] == month]
                if base_month.empty:
                    continue
                
                # Calculate monthly targets
                month_max = base_month['demand'].max() * growth_factor
                month_min = np.percentile(base_month['demand'], 5) * growth_factor
                month_range = month_max - month_min
                
                # Map each hour
                for hour in range(24):
                    hour_mask = month_mask & (profile_df['Hour'] == hour)
                    if not hour_mask.any():
                        continue
                    
                    # Get base hour pattern
                    base_hour = base_month[base_month['hour'] == hour]
                    if base_hour.empty:
                        continue
                    
                    # Use median normalized value for this hour
                    base_normalized = base_hour['normalized'].median()
                    
                    # Apply day type adjustments
                    day_types = profile_df.loc[hour_mask, 'day_type'].values
                    day_type_factors = np.ones(len(day_types))
                    
                    day_type_map = self.patterns.get('day_type_factors', {})
                    for i, dt in enumerate(day_types):
                        if dt in day_type_map:
                            day_type_factors[i] = day_type_map[dt]
                    
                    # Calculate final demand
                    adjusted_normalized = base_normalized * day_type_factors
                    demand[hour_mask] = month_min + adjusted_normalized * month_range
        
        profile_df['Demand_MW'] = demand
        
        return profile_df
    
    def _generate_stl_profile(self, profile_df):
        """Generate profile using STL decomposition method"""
        stl_components = self.patterns.get('stl', {})
        
        if not stl_components:
            print("  No STL components available, using normalized method", file=sys.stderr)
            return self._generate_normalized_profile(profile_df)
        
        # Get STL components
        seasonal_pattern = stl_components.get('seasonal', [])
        residual_std = stl_components.get('residual_std', 0)
        ...
        for year in range(self.start_year, self.end_year + 1):
            ...
            growth_factor = self._calculate_growth_factor(year)
            base_demand = 100 * growth_factor  # Base level
            ...
            for idx in year_indices:
                ...
                # Get seasonal component (168-hour cycle)
                seasonal_idx = (row['DayOfWeek'] * 24 + row['Hour']) % 168
                if seasonal_idx < len(seasonal_pattern):
                    seasonal = seasonal_pattern[seasonal_idx] * growth_factor
                else:
                    seasonal = 0
                
                # Apply day type adjustment
                day_type_factor = self.patterns.get('day_type_factors', {}).get(row['day_type'], 1.0)
                
                # Add controlled noise
                noise = np.random.normal(0, residual_std * 0.1)
                
                # Combine components
                demand[idx] = (base_demand + seasonal) * day_type_factor + noise
        
        profile_df['Demand_MW'] = np.maximum(demand, 10)  # Ensure positive values
        
        return profile_df
    
    def _extract_base_year_curve(self):
        """Extract base year curve efficiently"""
        historical_data = self.template_data.get('Past_Hourly_Demand', pd.DataFrame())
    ...
    def _apply_constraints_and_scaling(self, profile_df):
        """Apply final constraints and energy scaling"""
        # Apply annual energy targets
        for year in range(self.start_year, self.end_year + 1):
        ...
        # Apply monthly constraints if specified
        if self.monthly_constraints == 'excel':
            self._apply_excel_constraints(profile_df)
        
        return profile_df
    
    def _apply_excel_constraints(self, profile_df):
        """Apply Excel-based monthly constraints"""
        max_demand_constraints = self.template_data.get('max_demand', pd.DataFrame())
...
# Analysis output functions (unchanged to maintain output format)
def monthly_analysis(profile_df):
    """Generate monthly analysis (unchanged from original)"""
    monthly_analysis = ['Peak Demand', 'Min Demand', 'Average Demand', 'Monthly Load Factor','Total demand']
    result_rows = []
...
def seasonal_analysis(profile_df):
    """Generate seasonal analysis (unchanged from original)"""
    monthly_analysis = ['Peak Demand', 'Min Demand', 'Average Demand', 'Monthly Load Factor','Total Demand']
    result_rows = []
...
def daily_profile(profile_df):
    """Generate daily profile analysis (unchanged from original)"""
    monthly_analysis = ['Peak day Demand', 'Min Demand day', 'Average Demand']
    result_rows = []
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Optimized Load Profile Generation System')
    parser.add_argument('--config', required=True, help='Configuration JSON string or file path')
    ...
    try:
        ...
        print("="*80, file=sys.stderr)
        print("OPTIMIZED LOAD PROFILE GENERATION SYSTEM", file=sys.stderr)
        print("="*80, file=sys.stderr)
        
        # Load template data
        progress.start_process(6, "Optimized Load Profile Generation")
        progress.update_progress("Loading template data")
        ...
        # Extract patterns
        progress.update_progress("Extracting patterns")
        ...
        # Extract patterns
        pattern_extractor = OptimizedPatternExtractor(historical_data, method)
        patterns = pattern_extractor.extract_essential_patterns()
        
        # Generate profile
        progress.update_progress("Generating load profile")
        
        generator = OptimizedLoadProfileGenerator(config, patterns, template_data)
        generator.progress = progress
        profile_df = generator.generate_profile()
        ...
        # Save to Excel (unchanged output format)
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            # Main profile
            profile_df.to_excel(writer, sheet_name='Load_Profile', index=False)
            monthly_analysis(profile_df).to_excel(writer, sheet_name='Monthly_analysis', index=False)
            ...
            # Pattern information
            pattern_info = []
            if method == 'stl' and 'stl' in patterns:
                ...
                pattern_info.append({
                    'Pattern_Type': 'STL_Decomposition',
                    'Metric': 'Seasonal_Strength',
                    'Value': f"{stl_info.get('seasonal_strength', 0):.3f}"
                })
            else:
                pattern_info.append({
                    'Pattern_Type': 'Normalized_Base_Year',
