...
async def run_pypsa_model(request: RunModelRequest):
    ...
    try:
        ...
        master_sheet = project_path / "inputs" / "pypsa_input_template.xlsx"
        if not master_sheet.exists():
            raise HTTPException(
                status_code=400,
                detail=f"pypsa_input_template.xlsx not found in inputs folder. Please ensure input data exists."
            )
